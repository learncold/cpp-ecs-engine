...
- 관련 issue가 없고 docs/policy-only 또는 application-only 예외가 아닌 작업이면, 적절한 issue form으로 한국어 issue를 먼저 만든 뒤 branch 작업으로 들어간다.
...
- docs/policy-only 예외는 변경 경로가 `docs/`, `uml/`, `AGENTS.md`, `CONTRIBUTING.md`, PR/issue template, `pr-policy.yml`에만 한정될 때만 사용한다.
- application-only 예외는 변경 경로가 `src/application/` 및 application 파일을 `safecrowd_app`에 연결하기 위한 `CMakeLists.txt`에 한정될 때만 사용한다.
- docs/policy-only PR의 `Related Issue` 섹션은 `None (docs/policy-only PR)`로 남긴다.
- application-only PR의 `Related Issue` 섹션은 `None (application-only PR)`로 남긴다.
...
- application 폴더에 한정된 UI/앱 조립 변경은 별도 issue 없이 진행할 수 있다.
