...
- `WorldQuery`, `WorldResources`, `WorldCommands` : 컴포넌트 조회·값 접근 / 리소스 접근 / 구조 변경 요청을 분리한 접근면. 상위 계층은 `WorldQuery`를 직접 생성하지 않고 `EngineWorld::query()`로 획득한다. `WorldQuery::view<T...>()`는 가장 작은 packed component storage를 기준으로 필터링하되 반환 순서는 entity index 오름차순으로 유지하고, hot loop는 할당 없는 `forEach<T...>()`를 사용할 수 있다.
...
- 시나리오 실행기(`ScenarioSimulationRunner`)는 입력 delta를 엔진 `fixedDeltaTime` 단위로 분할해 fixed simulation phase를 통과시키고, 최종 sub-step 이후 최신 frame을 동기화한다.
- 시뮬레이션 규칙
- 표지판과 부분 지식 기반 local wayfinding 같은 occupant route 선택 규칙
- 위험 계산
...
- 현재 기본 DXF 경로는 in-repo 구현인 `DxfImportService -> RawImportModel -> GeometryNormalizer -> FacilityLayoutBuilder -> ImportValidationService`이며, 외부 SDK 없이도 rule 기반 분류와 reviewable fallback을 수행한다.
...
- 더 정밀한 DXF/IFC adapter 소스가 들어오면 같은 domain 경계 안에서 이 스위치를 재사용하고, 외부 SDK 헤더는 계속 `application`과 `engine` 밖으로 새지 않게 유지한다.
