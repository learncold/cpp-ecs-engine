...
이 문서는 SafeCrowd 앱에서 사용자가 실제로 만나는 화면, 기능 기준, 후속 UI 개선 대상을 한곳에 정리한다. 이 문서는 팀원이 "현재 어떤 화면 흐름이 기준인지", "이 버튼이 무엇을 하는지", "다음 Sprint에서 무엇을 보완해야 하는지"를 빠르게 확인하는 기준으로 쓴다.
...
- layout review 상태 저장 시 프로젝트 폴더에 `layout-review.json` 저장
- import provenance와 reimport 비교 정보 저장 시 프로젝트 폴더에 `import-artifacts.json` 저장
- workspace shell 상태 저장 시 프로젝트 폴더에 `workspace-state.json` 저장
...
- 저장 시점에 다음 조건을 만족하지 않으면 거부한다.
  ...
  - 비어 있거나, SafeCrowd 관리 파일(`safecrowd-project.json`, `layout.dxf`, `layout-review.json`, `import-artifacts.json`, `workspace-state.json`)만 존재해야 한다.
...
- 프로젝트명 입력
- layout DXF 파일 선택
- layout 파일 확장자가 `.dxf`가 아니면 프로젝트 생성을 거부
- 프로젝트 저장 폴더 선택
...
- DXF import 결과 표시
- import 상태와 raw/canonical/layout/issue 요약 표시
- 원본 DXF가 남아 있는 일반 프로젝트에서 `Reimport Layout` 실행
- reimport 이후 이전 review layout과 새 import layout의 added/removed/changed 요약 표시
...
- issue inspector에서 source/target, confidence, repair suggestion 표시
...
- 중앙 Layout Preview
- 우측 선택 inspector
- 우측 import status summary
- 우측 `Reimport Layout` action
- 우측 approval action
- 하단 review 상태 영역
...
현재 우측 issue inspector 표시 대상:

- issue code와 severity
- message
- source id와 target id
- confidence
- repair suggestion
...
- [x] DXF가 아닌 파일 선택 방지 강화
- [ ] 같은 폴더에 기존 프로젝트가 있을 때 덮어쓰기 확인
- [x] DXF import 진행 상태 표시
- [ ] import 실패 상세 메시지 또는 전용 화면 제공
- [x] Layout Review issue 클릭과 canvas focus 연동 강화
- [ ] 선택 요소의 상세 편집 inspector 보강
- [ ] warning/info 승인 또는 무시 상태 표시
- [x] issue별 repair suggestion 표시
...
- 작업 단위, 상태, PR 연결 방식은 현재 기여 규칙을 따른다.
