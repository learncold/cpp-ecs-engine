...
## GitHub Workflow
- Development is no longer based on mandatory GitHub issue creation or issue linking.
- Do not check for, create, or link GitHub issues before starting implementation unless the user explicitly asks for issue/project management work.
...
- `main` handling:
  ...
  - docs/policy-only changes limited to `docs/`, `uml/`, `AGENTS.md`, `CONTRIBUTING.md`, PR templates, or PR policy workflow files may be pushed directly to `main` by maintainers
  - application-only changes limited to `src/application/` and app target wiring in `CMakeLists.txt` may be pushed directly to `main` by maintainers
